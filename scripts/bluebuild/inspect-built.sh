...
mkdir -p .bluebuild
measure=(python3 "$(dirname "${BASH_SOURCE[0]}")/measure.py")
...
if [[ $published == true ]]; then
  ...
  candidate_digest=$("${measure[@]}" registry-resolution timeout 150s skopeo --command-timeout 30s inspect --retry-times 3 --no-tags \
  ...
  for attempt in 1 2 3; do
    ...
    if "${measure[@]}" image-pull timeout 15m docker pull "$image"; then
      break
    else
      status=$?
    fi
  ...
  "${measure[@]}" signature timeout 150s cosign verify --key cosign.pub "$image" >".bluebuild/$profile-signature.json"
else
...
"${measure[@]}" runtime-checks docker run --rm --privileged --entrypoint /bin/bash \
...
if [[ -n ${GITHUB_OUTPUT:-} && $published == true ]]; then
  printf 'digest=%s\n' "${image##*@}" >>"$GITHUB_OUTPUT"
fi
