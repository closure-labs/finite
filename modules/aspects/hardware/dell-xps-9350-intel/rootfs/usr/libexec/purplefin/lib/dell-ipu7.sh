...
purplefin_dell_ipu7_fix_pack_repo() {
	printf '%s\n' 'https://github.com/jibsta210/svp7500-camera-fix-pack'
}

purplefin_dell_ipu7_fix_pack_version() {
	printf '%s\n' 'v1.0.2'
}

purplefin_dell_ipu7_fix_pack_ref() {
...
purplefin_dell_ipu7_assert_replaceable_module() {
	local release="$1"
	local module_file="$2"
	local builtins="/usr/lib/modules/${release}/modules.builtin"

	if [[ -f "${builtins}" ]] && grep -qE "/${module_file}[.]ko$" "${builtins}"; then
		purplefin_dell_ipu7_log "${module_file} is built into ${release}; the SVP7500 replacement cannot take precedence"
		return 1
	fi
}

purplefin_dell_ipu7_int3472_patch_needed() {
	local release="$1"
