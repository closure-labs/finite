...
The repository-local action invokes the unified Image Builder CLI directly and
shares the same build, cache, diagnostics, and smoke-test path between pull
requests and published installer runs.
...
The installer consumes a published, signed Purplefin OCI digest. The
...
before enabling auto-merge. The immutable digest recorded by that pull request
constructs each installer artifact.
