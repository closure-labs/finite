...
`ci-unattended.toml.in` to create a separate test-only ISO. It performs a
non-interactive Kickstart installation onto an ephemeral disk, reboots from the
disk, and waits for `PURPLEFIN_INSTALLED_READY=1`. The test ISO and disk are not
published.
