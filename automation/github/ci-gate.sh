...
require_selected_result base-sbom "${HAS_ROOT_BASE:-false}" "${BASE_SBOM_RESULT:?}"
require_selected_result hardware-sbom "${HAS_HARDWARE:-false}" "${HARDWARE_SBOM_RESULT:?}"
require_selected_result role-sbom "${HAS_ROLES:-false}" "${ROLE_SBOM_RESULT:?}"
