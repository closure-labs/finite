...
base_overlay="${build_root}/overlays/base"

cp -a "${base_overlay}/files/." /
install -D -m 0644 "${base_overlay}/manifests/Brewfile" \
	/usr/share/purplefin/manifests/Brewfile
install -D -m 0644 "${base_overlay}/manifests/flatpaks.preinstall" \
...
bash "${build_root}/packages/bitwarden-cli/install.sh"
