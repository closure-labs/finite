...
Installer-affecting pull requests run the reusable build through `CI gate`;
weekly trusted runs refresh matching registry-cache layers. Every completed ISO
is smoke-booted with QEMU and described by an attested component manifest.
...
the Home Manager activation packages in its OCI root.
